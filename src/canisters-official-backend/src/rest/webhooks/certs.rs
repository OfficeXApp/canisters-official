...
use crate::rest::webhooks::route::WEBHOOKS_LIST_PATH;
...
lazy_static! {
    pub static ref WEBHOOKS_TREE_PATH: HttpCertificationPath<'static> = HttpCertificationPath::exact(WEBHOOKS_LIST_PATH);
    static ref NOT_FOUND_TREE_PATH: HttpCertificationPath<'static> = HttpCertificationPath::wildcard(NOT_FOUND_PATH);

    static ref WEBHOOKS_CEL_EXPR_DEF: DefaultFullCelExpression<'static> = DefaultCelBuilder::full_certification()
        ...
        .build();
    static ref WEBHOOKS_CEL_EXPR: String = WEBHOOKS_CEL_EXPR_DEF.to_string();
