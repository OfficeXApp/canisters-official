...
pub mod drives_handlers {
    ...
    pub async fn about_drive_handler<'a, 'k, 'v>(request: &'a HttpRequest<'a>, params: &'a Params<'k, 'v>) -> HttpResponse<'static> {
        ...
        let version = VERSION.with(|v| v.borrow().get().clone());
    ...
    pub async fn update_allowed_domains_drive_handler<'a, 'k, 'v>(request: &'a HttpRequest<'a>, params: &'a Params<'k, 'v>) -> HttpResponse<'static> {
        // Authenticate request
        let requester_api_key = match authenticate_request(request) {
            Some(key) => key,
            None => return create_auth_error_response(),
        ...
        // Parse request body
        let body: &[u8] = request.body();
        let update_request = match serde_json::from_slice::<UpdateAllowedDomainsDriveRequestBody>(body) {
            Ok(req) => req,
            Err(_) => return create_response(
                StatusCode::BAD_REQUEST,
                ErrorResponse::err(400, "Invalid request format".to_string()).encode()
        ...
        if let Err(validation_error) = update_request.validate_body() {
            return create_response(
                StatusCode::BAD_REQUEST,
                ErrorResponse::err(400, format!("Validation error: {}: {}", 
                    validation_error.field, validation_error.message)).encode()
        ...
        create_response(
            StatusCode::OK,
            UpdateAllowedDomainsDriveResponse::ok(&UpdateAllowedDomainsDriveResponseData {
                success: true,
                message: "Allowed domains updated successfully".to_string(),
            }).encode()
        )
    }
    ...
    pub async fn redeem_organization_drive_handler<'a, 'k, 'v>(request: &'a HttpRequest<'a>, params: &'a Params<'k, 'v>) -> HttpResponse<'static> {
        ...
        let drive_id = DRIVE_ID.with(|id| id.clone());
        let canister_id = CANISTER_ID.with(|id| id.0.clone());
        let host_url = URL_ENDPOINT.with(|url| url.borrow().get().0.clone());
        ...
        let admin_login_password = format!("{}:{}@{}", drive_id, admin_api_key, host_url);
        let auto_login_url = "".to_string();
