// src/rest/group_invites/certs.rs
...
use crate::rest::group_invites::route::GROUP_INVITES_LIST_PATH;
...
lazy_static! {
    pub static ref GROUP_INVITES_TREE_PATH: HttpCertificationPath<'static> = HttpCertificationPath::exact(GROUP_INVITES_LIST_PATH);
    static ref NOT_FOUND_TREE_PATH: HttpCertificationPath<'static> = HttpCertificationPath::wildcard(NOT_FOUND_PATH);

    static ref GROUP_INVITES_CEL_EXPR_DEF: DefaultFullCelExpression<'static> = DefaultCelBuilder::full_certification()
        ...
        .build();
    static ref GROUP_INVITES_CEL_EXPR: String = GROUP_INVITES_CEL_EXPR_DEF.to_string();
