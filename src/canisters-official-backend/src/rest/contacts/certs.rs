...
use crate::rest::contacts::route::CONTACTS_LIST_PATH;
...
lazy_static! {
    pub static ref CONTACTS_TREE_PATH: HttpCertificationPath<'static> = HttpCertificationPath::exact(CONTACTS_LIST_PATH);
    static ref NOT_FOUND_TREE_PATH: HttpCertificationPath<'static> = HttpCertificationPath::wildcard(NOT_FOUND_PATH);

    static ref CONTACTS_CEL_EXPR_DEF: DefaultFullCelExpression<'static> = DefaultCelBuilder::full_certification()
        ...
        .build();
    static ref CONTACTS_CEL_EXPR: String = CONTACTS_CEL_EXPR_DEF.to_string();
