...
use crate::rest::directory::route::DIRECTORYS_LIST_PATH;
...
lazy_static! {
    pub static ref DIRECTORYS_TREE_PATH: HttpCertificationPath<'static> = HttpCertificationPath::exact(DIRECTORYS_LIST_PATH);
    static ref NOT_FOUND_TREE_PATH: HttpCertificationPath<'static> = HttpCertificationPath::wildcard(NOT_FOUND_PATH);

    static ref DIRECTORYS_CEL_EXPR_DEF: DefaultFullCelExpression<'static> = DefaultCelBuilder::full_certification()
        ...
        .build();
    static ref DIRECTORYS_CEL_EXPR: String = DIRECTORYS_CEL_EXPR_DEF.to_string();
